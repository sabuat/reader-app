    ...
    <html lang={lang || "es"} className="bg-brand-bg dark:bg-[#121212]">
      ...
      <body className="bg-brand-bg dark:bg-[#121212] text-brand-dark dark:text-gray-200 min-h-[100dvh] flex flex-col overflow-x-hidden antialiased transition-colors duration-500">
        ...
        <ThemeProvider>
          ...
          {showNav && !isReadingMode && (
            <nav className="fixed bottom-0 w-full bg-brand-bg dark:bg-[#121212] backdrop-blur-lg dark:backdrop-blur-none border-t border-brand-gold/10 dark:border-brand-gold/20 px-8 flex justify-between items-center z-40 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] dark:shadow-none transition-colors duration-500 pb-[calc(1rem+env(safe-area-inset-bottom))] h-[calc(5rem+env(safe-area-inset-bottom))]">
              ...
              <Link href="/lista" className={`flex flex-col items-center gap-1 transition-all active:scale-90 ${isActive('/lista') ? 'text-brand-dark-blue dark:text-brand-gold' : 'text-gray-400 dark:text-gray-500'}`}>
                <Bookmark size={22} />
                <span className="text-[10px] font-bold uppercase tracking-tighter">{t('box.my_list')}</span>
              </Link>
